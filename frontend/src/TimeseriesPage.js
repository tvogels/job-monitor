                    ...
                    <div style={{ display: "flex", flexGrow: 1, flexDirection: "column", minHeight: 0, minWidth: 0 }}>
                        <div style={{ display: "flex" }} tabIndex={0} onKeyDown={handleKeyEvent}>
                            ...
                            <FormGroup label="State">
                                <Button icon="clipboard" onClick={stateToClipboard} />
                            </FormGroup>
                        </div>
                        <div
                            style={{
...
/>
    ...
    return (
        <Query
            query={DATA_QUERY}
            pollInterval={10000}
            variables={{ ids: jobIds, measurement: measurementQuery.join("|"), tags: tagQuery.join(",") }}
        >
            {({ loading, error, data }) => {
                if (error) return <p>Error :( {JSON.stringify(error)}</p>;
                ...
                const patternScale = pattern
                    ? scaleOrdinal([null, [5, 5], [2, 2], [8, 4]]).domain(patternDomain)
                    : null;
                        ...
                        {pattern ? (
                            ...
                            >
                                <div style={{ marginRight: "1em" }}>
                                    {patternLegendName || pattern.replace(/[-_]/g, " ")}:
                                </div>
                                    ...
                                    <div key={pattern} style={{ marginRight: "1.5em" }}>
                                        <svg width={20} height={9}>
                                            <line
                                                ...
                                                strokeWidth={2}
                                                strokeDasharray={patternScale(pattern)}
                                            />
                                        </svg>{" "}
                                        {pattern}
                                    </div>
                        ...
                        >
                            {(parent) => {
                                const cellHeight = Math.max(
                                    0,
                                    Math.round(
                                        (parent.height - margin.top - margin.bottom + margin.row) / nrows - margin.row
                                    )
                                );
                                const cellWidth = Math.max(
                                    0,
                                    Math.round(
                                        (parent.width - margin.left - margin.right + margin.col) / ncols - margin.col
                                    )
                                );
                                ...
                                yScale = yScale.domain(yDomain).rangeRound([cellHeight, 0]);
                                        ...
                                        {rowDomain.map((rowValue, rowIdx) => (
                                            <Group key={rowIdx} top={margin.top + rowIdx * (margin.row + cellHeight)}>
                                                {colDomain.map((colValue, colIdx) => (
                                                    <Group
                                                        key={colIdx}
                                                        left={margin.left + colIdx * (margin.col + cellWidth)}
                                                    >
                                                        <rect
                                                            fill="rgb(62, 78, 91)"
                                                            width={cellWidth}
                                                        ...
                                                        {crossHair ? (
                                                            <Line
                                                                ...
                                                                to={
                                                                    new Point({ x: xScale(crossHair.x), y: cellHeight })
                                                                }
                                                            />
                                                        ) : null}
                                                        {curves
                                                            .filter(
                                                                (e) =>
                                                                    e.properties[row] === rowValue &&
                                                                    e.properties[col] === colValue
                                                            )
                                                            .map((entry) => (
                                                                <Group key={entry.entryId}>
                                                                    <LinePath
                                                                        ...
                                                                        stroke={
                                                                            hue
                                                                                ? hueScale(entry.properties[hue])
                                                                                : "rgb(221, 226, 229)"
                                                                        }
                                                                        strokeDasharray={
                                                                            pattern
                                                                                ? patternScale(
                                                                                      entry.properties[pattern]
                                                                                  )
                                                                                : null
                                                                        }
                                                                        opacity={lineOpacity}
                                                                    ...
                                                                    />
                                                                    <NaNIndicator
                                                                        data={aggregateBy(entry.values, (d) =>
                                                                            x(d)
                                                                        ).filter((d) => y(d) == null)}
                                                                        x={(d) => xScale(x(d))}
                                                                        y={(d) => yScale(y(d))}
        ...
        <Group fill={color} opacity={opacity}>
            {data.map((d, i) => (
                <polygon
                    key={i}
                    points="0 -5 5 5 -5 5"
                    transform={`translate(${x(d) + 5 * (Math.random() - 0.5)}, ${y(d) -
                        5 +
                        5 * (Math.random() - 0.5)})`}
                ></polygon>
            ))}
